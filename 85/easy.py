...
then calculates the sums for each row and column, optionally
outputting them...
...
def swap(matrix):
    xirtam = [[None] * len(matrix) for _ in range(len(matrix[0]))]
...
def main(args):
    matrix = [[int(value) for value in line.split()] for line in sys.stdin]
    xirtam = [[0] * len(matrix) for _ in range(len(matrix[0]))]
    for i, row in enumerate(matrix):
        for j, cell in enumerate(row):
            xirtam[j][i] = cell
    xirtam = swap(matrix)

    print 'Rows:', ' '.join(str(sum(row)) for row in matrix)
    print 'Columns:', ' '.join(str(sum(column)) for column in xirtam)

    matrix.sort(key=lambda row: sum(row))
    for row in matrix:
        print ' '.join(str(cell) for cell in row)

    xirtam.sort(key=lambda column: sum(column))
    matrix = swap(xirtam)
    for row in matrix:
        print ' '.join(str(cell) for cell in row)
